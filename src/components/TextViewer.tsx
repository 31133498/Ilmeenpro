...
function WordSpan({ word, entry, isMobile, onTap }: {
  ...
  entry?: WordEntry
  isMobile: boolean
  onTap: (e: WordEntry) => void
}) {
  const [hovered, setHovered] = useState(false)
  const spanRef = useRef<HTMLSpanElement>(null)
  const [tooltipLeft, setTooltipLeft] = useState<number | string>('50%')
  ...
  const handleMouseEnter = () => {
    if (!isMobile && entry) {
      ...
      if (spanRef.current) {
        ...
        if (centerX - tipWidth / 2 < 8) {
          // Too close to left edge — anchor left
          setTooltipLeft(0)
          setTooltipTransform('translateX(0)')
        } else if (centerX + tipWidth / 2 > window.innerWidth - 8) {
    ...
    <span style={{ position: 'relative', display: 'inline-block', margin: '0 2px' }}>
      <span
        ...
        style={{
          ...
          borderBottom: entry ? '1px dotted rgba(201,168,76,0.5)' : 'none',
        }}
      >
        {word}
      </span>
      ...
      {hovered && entry && (
        <div style={{
          ...
          bottom: 'calc(100% + 8px)',
          left: tooltipLeft,
          right: tooltipLeft === 'auto' ? 0 : undefined,
          transform: tooltipTransform,
          background: '#fff',
          border: '1px solid rgba(201,168,76,0.4)',
          borderRadius: 12,
          padding: '10px 14px',
          boxShadow: '0 4px 24px rgba(26,22,17,0.14)',
          zIndex: 200,
          minWidth: 140,
          maxWidth: 200,
          pointerEvents: 'none',
          direction: 'ltr',  // tooltip content always LTR
        }}>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#1A1611', fontWeight: 500 }}>
            {entry.meaning}
          </div>
          {entry.root && (
            <div style={{ fontFamily: "'Noto Naskh Arabic',serif", fontSize: 15, color: '#C9A84C', direction: 'rtl', marginTop: 3 }}>
              {entry.root}
            </div>
          )}
          <span style={{
...
export default function TextViewer({ session, focusMode, onFocusLineChange }: Props) {
  ...
  // Rebuild word index whenever session changes
  useEffect(() => {
  ...
  const playLine = useCallback(async (idx: number) => {
    ...
    try {
      const src = await speakArabic(lines[idx])
  ...
  const startLoop = useCallback(async (idx: number) => {
  ...
  }, [focusMode, activeLine, lines.length, navigate])

  // Enter focus mode — auto-navigate to line 0 if none active
  useEffect(() => {
    if (focusMode && activeLine === null) navigate(0)
  }, [focusMode]) // eslint-disable-line react-hooks/exhaustive-deps

  // Touch swipe for focus mode
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, justifyContent: 'flex-end' }}>
        <button onClick={prevFont} style={{ minWidth: 44, minHeight: 44, borderRadius: 8, border: '1px solid rgba(201,168,76,0.3)', background: 'none', fontSize: 16, cursor: 'pointer', color: '#7A6E62' }}>A−</button>
        <span style={{ fontSize: 12, color: '#7A6E62' }}>{fontSize}px</span>
        <button onClick={nextFont} style={{ minWidth: 44, minHeight: 44, borderRadius: 8, border: '1px solid rgba(201,168,76,0.3)', background: 'none', fontSize: 16, cursor: 'pointer', color: '#7A6E62' }}>A+</button>
      </div>
              ...
              >
                {words.map((w, wi) => (
                  <WordSpan
                    key={wi}
                    word={w}
                    entry={wordIndex.current.get(w)}
                    isMobile={isMobile}
                    onTap={setBottomSheetEntry}
                  />
                ))}
                {isMemorized && <span style={{ marginRight: 8, color: '#2d7a4f', fontSize: 16 }}>✓</span>}
              </div>

              {/* Mini control bar — shown when line is active */}
              {isActive && (
                <div style={{
                  display: 'flex', gap: 8, justifyContent: 'flex-end',
                  padding: '4px 8px 8px', flexWrap: 'wrap',
                }}>
                  <button
                    onClick={() => playLine(idx)}
                    style={{ minHeight: 44, padding: '6px 14px', borderRadius: 100, background: '#1A1611', color: '#FDFBF7', fontSize: 13, fontWeight: 500, border: 'none', cursor: 'pointer' }}
                  >▶ Play line</button>
                  <button
                    onClick={() => startLoop(idx)}
                    style={{ minHeight: 44, padding: '6px 14px', borderRadius: 100, background: isLooping ? '#C9A84C' : 'transparent', color: isLooping ? '#1A1611' : '#7A6E62', fontSize: 13, border: '1px solid rgba(201,168,76,0.4)', cursor: 'pointer' }}
                  >🔁 {isLooping ? 'Stop loop' : 'Loop'}</button>
                  <button
                    onClick={() => toggleMemorized(idx)}
                    style={{ minHeight: 44, padding: '6px 14px', borderRadius: 100, background: isMemorized ? '#e6f4ed' : 'transparent', color: isMemorized ? '#2d7a4f' : '#7A6E62', fontSize: 13, border: `1px solid ${isMemorized ? '#2d7a4f' : 'rgba(201,168,76,0.4)'}`, cursor: 'pointer' }}
                  >{isMemorized ? '✓ Memorized' : '✓ Memorized?'}</button>
                </div>
              )}
            </div>
          )
      ...
      {/* Mobile bottom sheet for word info */}
